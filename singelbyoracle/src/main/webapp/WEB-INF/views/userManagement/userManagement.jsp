...
$(function() {
	...
	$("#multiforcedExit").click(function() {
		if($("#normal tbody input[type=checkbox]:checked").length>0){
			...
			"<c:url value='/userManagementMultiforcedExit'/>");	
	...
	$("#multiCancle").click(function() {
		if($("#event tbody input[type=checkbox]:checked").length>0){
			...
			"<c:url value='/userManagementMultiCancle'/>");	
	...
	$("#normal tbody button").click(function() {
		...
		if(confirm("해당 회원을 강퇴 하시겠습니까?")){
			$.ajax({
				url: "<c:url value='/userManagementForcedExit'/>",
				type:"post",
				data: {
					userid : userid
				},
	...
	$("#event tbody button").click(function() {
		...
		if(confirm("해당 회원을 탈퇴를 취소 하시겠습니까?")){
			$.ajax({
				url: "<c:url value='/userManagementCancle'/>",
				type:"post",
				data: {
					userid : userid
				},
...
<form action="<c:url value='/userManagement'/>" 
	...
	<colgroup>
		...
		<c:if test="${!empty list2 }">
			...
			<c:forEach var="vo2" items="${list2 }">
				...
				<td>${vo2.email1 }@${vo2.email2 }</td>
				<td><fmt:formatDate value="${vo2.outDate }" pattern="yyyy-MM-dd"/>
				</td>
				<td>
